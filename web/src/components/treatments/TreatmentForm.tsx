...
export default function TreatmentForm({ patients, radioisotopes, units, watches, doctors }: Props) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('Treatment').insert({
      ...
      // Mark watch as in use
      await supabase
        .from('Dispositiu_Rellotge')
        .update({ estat: 'EN_US' })
        .eq('id', Number(form.rellotge_id));
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6">
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          ...
          <div>
            ...
            <select
              value={form.facultatiu_id}
              onChange={(e) => handleChange('facultatiu_id', e.target.value)}
              className={selectClass}
              required
            >
          ...
          <div>
            ...
            <select
              value={form.radioisotope_id}
              onChange={(e) => handleChange('radioisotope_id', e.target.value)}
              className={selectClass}
              required
            >
          ...
          <div>
            ...
            <input
              ...
              step="0.01"
              min="0"
              value={form.initial_radiation}
              onChange={(e) => handleChange('initial_radiation', e.target.value)}
              placeholder="Ej: 3700"
          ...
          <div>
            ...
            <input
              ...
              min="1"
              max="365"
              value={form.isolation_days}
              onChange={(e) => handleChange('isolation_days', e.target.value)}
              placeholder="Ej: 7"
              className={inputClass}
              required
            />
          ...
          <div>
            ...
            <select
              value={form.rellotge_id}
              onChange={(e) => handleChange('rellotge_id', e.target.value)}
              className={selectClass}
              required
            >
              ...
              {watches.map((w) => (
                <option key={w.id} value={w.id}>
                  {w.model} - {w.mac_address} ({w.nivell_bateria}%)
                </option>
              ))}
            </select>
            {watches.length === 0 && (
              <p className="mt-1 text-xs text-alert-amber">No hay relojes disponibles</p>
            )}
          </div>
        ...
        {expectedEndDate && (
          <div className="mt-5 rounded-lg bg-radix-50 p-4">
            <p className="text-sm text-radix-700">
              ...
              {new Date(expectedEndDate).toLocaleDateString('es-ES', {
                day: '2-digit',
                month: 'long',
                year: 'numeric',
              })}
